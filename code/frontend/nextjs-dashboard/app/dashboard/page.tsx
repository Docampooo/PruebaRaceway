...
//TODOS
//1 --> Asociar funciones de muestra a la API

import { useState } from 'react';
import { lusitana } from '@/app/ui/fonts';
import { FuncionMotor } from '@/app/tipos/raceway';
import { motorFunctions, muestraFunctions, funciones, muestraFunciones } from '@/app/data/raceway';
    ...
    <main className="min-h-screen bg-white px-4 py-8">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-4 flex items-start gap-4">
          ...
          <div className="hidden h-auto w-px self-stretch bg-gray-200 sm:block" />

          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            {muestraFunctions.map((fn) => (
              <FuncionCard
                key={fn.id}
                fn={fn}
                cargando={cargando === fn.id}
        ...
        {/* ── Toma de Muestras ── */}
        <div className="mb-6 text-center">
          <p className={`${lusitana.className} text-3xl font-bold text-green-700`}>
            Toma de Muestras
          </p>
          <p className="mt-2 text-sm uppercase tracking-widest text-gray-400">
            Recogida manual y automatica de microalgas
          </p>
          <div className="mx-auto mt-3 h-px w-16 bg-green-200" />
        </div>
        <div className="space-y-4">
          {muestraFunciones.map((fn, i) => (
            <FuncionInfoCard key={fn.id} fn={fn} index={i} />
          ))}
        </div>
